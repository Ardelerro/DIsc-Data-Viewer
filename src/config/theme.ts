...
// Use these in inline style props (Recharts, canvas, framer-motion style={{}}).
// For Tailwind className, use bg-[var(--color-*)] directly or via recipes below.
export const C = {
  bg:                "var(--color-bg)",
...
// ─── Typography ────────────────────────────────────────────────────────────
// Edit font sizes in design.cjs — the Vite plugin turns them into CSS variables
// and @theme entries. FONT.size refs here point at those variables at runtime.
export const FONT = {
...
// ─── Icon Sizes (Tailwind class strings) ───────────────────────────────────
export const ICON = {
  sm:  "w-3.5 h-3.5",  // 14px
  md:  "w-4 h-4",      // 16px — standard toolbar / card icons
  lg:  "w-5 h-5",      // 20px — modal close, nav arrows
  xl:  "w-8 h-8",      // 32px — mobile FAB
} as const;

// ─── Border Radius (Tailwind class strings) ────────────────────────────────
export const RADIUS = {
  sm:   "rounded-md",   // 6px  — buttons, tags, small elements
  md:   "rounded-lg",   // 8px  — inputs, tooltips
  lg:   "rounded-xl",   // 12px — cards, modals, panels
  full: "rounded-full", // pill — badges, avatars, pills
} as const;

// ─── Achievement Tier Styles ───────────────────────────────────────────────
...
export const FEEDBACK = {
  error: {
    container: "bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg",
...
export const TOOLTIP = {
  panel: "bg-[var(--color-tooltip-bg)] text-[var(--color-tooltip-text)] rounded-xl px-3 py-2.5 shadow-xl",
  arrow: "w-2 h-2 bg-[var(--color-tooltip-bg)] rotate-45 -mt-1",
} as const;

/** Personality archetype badge */
export const PERSONALITY = {
  badge: "flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-[var(--color-personality-bg)] border border-[var(--color-personality-border)] cursor-default select-none",
...
export const TIME_PILL = {
  ...
  inactive: "bg-[var(--color-surface-raised)] border border-[var(--color-border)] text-[var(--color-text-2)] hover:border-[var(--color-border-hover)] cursor-pointer",
  disabled: "bg-[var(--color-surface-raised)] border border-[var(--color-border)] text-[var(--color-text-3)] cursor-not-allowed",
} as const;
