...
function stripEnds(s: string): string {
  ...
  while (end > start) {
    ...
    if (c === 46 || c === 44 || c === 33 || c === 63) end--;
    else break;
  }
  return start === 0 && end === s.length ? s : s.slice(start, end);
}

function removeNonAlnum(s: string): string {
  let i = 0;
  for (; i < s.length; i++) if (!isAlnumCode(s.charCodeAt(i))) break;
  if (i === s.length) return s;
  let out = s.slice(0, i);
  for (; i < s.length; i++) {
    if (isAlnumCode(s.charCodeAt(i))) out += s[i];
  }
  return out;
}
...
function tokenize(cleaned: string): { tokens: string[]; words: string[] } {
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    for (let k = 0; k < part.length; k++) {
      if (!isAlnumCode(part.charCodeAt(k))) {
        allAlnum = false;
        break;
      }
    }
    ...
    const stripped = stripEnds(part);
    if (stripped && EMOTICONS[stripped] !== undefined) {
      tokens.push(stripped);
      continue;
    }

    const word = removeNonAlnum(part);
    if (word) {
      tokens.push(word);
...
export function tokenizeWords(text: string): string[] {
  ...
  return tokenize(text.replace(NOISE_RE, " ")).words;
}
...
export function analyzeText(text: string): SentimentResult {
  ...
  const lower = new Array<string>(tokens.length);
  for (let i = 0; i < tokens.length; i++) lower[i] = tokens[i].toLowerCase();

  for (let i = 0; i < tokens.length; i++) {
    const tok = tokens[i];
    let v = EMOTICONS[tok];
    if (v === undefined) v = LEXICON[lower[i]] ?? 0;
    if (v === 0) continue;
  ...
  for (let i = 0; i < tokens.length; i++) {
    if (lower[i] === "but") {
      ...
      for (let k = i + 1; k < valences.length; k++)
        valences[k] *= BUT_LATER_WEIGHT;
      break;
    }
  }
  ...
  const puncBoost =
    meta.exclaim * EXCLAMATION_PER + meta.question * QUESTION_PER;
  if (sumValence > 0) sumValence += puncBoost;
  else if (sumValence < 0) sumValence -= puncBoost;

  const compound =
    sumValence / Math.sqrt(sumValence * sumValence + COMPOUND_ALPHA);
